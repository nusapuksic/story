...
// SummaryCharacterFinalState is final-state context for a book-level character
// carried by a generated summary.
...
type SummaryContext struct {
	...
	CharacterFinalStates []SummaryCharacterFinalState
}

// CharacterRoleEvidence is scene-scoped support for a character role.
type CharacterRoleEvidence struct {
	SceneID string
	Reason  string
}

// CharacterRoleContext is a compiled character/principal role made available
// to ask prompts.
type CharacterRoleContext struct {
	RecordType      string
	CharacterID     string
	...
	Confidence      float64
	Rationale       string
	Evidence        []CharacterRoleEvidence
	Status          string
}

// EvidenceDigest is an ephemeral ask-run condensation of lower-level evidence.
// Its ID is generated by deterministic code for the current query run only.
...
type Answer struct {
	...
	// RecordsUsed contains validated higher-level records used for answers
	// where paragraph citations are too narrow or misleading.
	RecordsUsed []string `json:"records_used,omitempty"`
...
type Options struct {
	...
	// CharacterRoles are generated book-level character role records to include
	// for character inventory, profile, and development questions.
...
type rawAnswer struct {
	...
	RecordsUsed   []string `json:"records_used"`
...
func Ask(
	...
	intent := classifyQueryIntent(question, opts.Mode, opts.ChapterID)

	// Step 1: Retrieve relevant scene cards and paragraphs via FTS.
	ret, err := retrieval.Search(st, question, retrieval.Options{
		ChapterID:             opts.ChapterID,
		MaxParagraphs:         retrievalParagraphLimit(opts.MaxEvidence, intent),
		MaxSceneCards:         10,
	...
	packet, err := buildEvidencePacket(ctx, st, prov, model, question, opts, intent, ret, paragraphs, entityContext, usedParagraphFallback, cardPolicy)
	if err != nil {
		return nil, err
	}

	// Build sets of valid IDs for citation and record validation.
	validIDs := make(map[string]string, len(packet.Paragraphs)) // id -> chapter_id
	for _, p := range packet.Paragraphs {
		validIDs[p.ID] = p.ChapterID
	}
	validRecords := packet.validRecordIDs()
	...
	userPrompt := buildUserPrompt(question, opts.Mode, packet)
	...
	validatedRecords := validateRecordIDs(raw.RecordsUsed, validRecords)
	if len(validatedRecords) == 0 && len(validatedEvidence) == 0 && defaultRecordsUsedForIntent(intent) {
		validatedRecords = packet.RecordsUsed
	}
